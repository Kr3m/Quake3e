...
// Tracks the most recent syscall number dispatched; set by each VM's syscall handler.
intptr_t vm_lastSyscall = 0;
...
void VM_CheckBounds( const vm_t *vm, unsigned int address, unsigned int length )
{
	//if ( !vm->entryPoint )
	{
		...
		{
			Com_Printf( S_COLOR_RED "VM_CheckBounds FAIL: vm=%s syscall=%ld addr=0x%X len=0x%X dataMask=0x%X addr+len=0x%X\n",
				vm->name, (long)vm_lastSyscall, address, length, vm->dataMask, address + length );
...
void VM_CheckBounds2( const vm_t *vm, unsigned int addr1, unsigned int addr2, unsigned int length )
{
	//if ( !vm->entryPoint )
	{
		...
		{
			Com_Printf( S_COLOR_RED "VM_CheckBounds2 FAIL: vm=%s syscall=%ld addr1=0x%X addr2=0x%X len=0x%X dataMask=0x%X\n",
				vm->name, (long)vm_lastSyscall, addr1, addr2, length, vm->dataMask );
			Com_Error( ERR_DROP, "program tried to bypass data segment bounds" );
		}
	}
}
